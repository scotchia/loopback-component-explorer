...
// AESG Configuration
let AESG = {
  order:["Exam","Assignment","Syllabus","LectureProposal","Lecture","Project","ProjectTeam","Setting","File","Organization","Group","SystemUserData","SystemUser"],
  hideArray:[
    "SystemAccess",
    "ProjectAccess",
    "Queue"
  ],
  mainTitle:"Main Models",
...
function changeTitle(){
  $('#api_info > div.info_title').text(AESG.mainTitle);
}
function addDevider(Title){
  $("#resources li#resource_Project").after('<li id="resource_devider"><div class="info_title">'+AESG.componentsTitle+'</div></li>');
}
function AESGsorter(a,b){
  return AESG.order.indexOf(b.name)-AESG.order.indexOf(a.name);
}
...
$(function() {
  ...
  function loadSwaggerUi(config) {
    ...
    // hack to filter out internal endpoints, operations and parameters before rendering
      ...
      function filterInternal(x) { return AESG.hideArray.indexOf(x.name)<0; };
